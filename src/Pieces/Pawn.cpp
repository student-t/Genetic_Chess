...
Pawn::Pawn(Color color_in) : Piece(color_in, "P")
{
    ...
    for(char file = 'a'; file <= 'h'; ++file)
    {
        ...
        {
            add_legal_move(std::make_unique<Pawn_Move>(color_in, file, rank));
        }
    }

    for(char file = 'a'; file <= 'h'; ++file)
    {
        add_legal_move(std::make_unique<Pawn_Double_Move>(color_in, file));
    }
    ...
    for(auto dir : {RIGHT, LEFT})
    {
        ...
        for(char file = first_file; file <= last_file; ++file)
        {
            for(int rank = base_rank; rank != no_normal_move_rank; rank += direction)
            {
                add_legal_move(std::make_unique<Pawn_Capture>(color_in, dir, file, rank));
            }
        }
        ...
        for(char file = first_file; file <= last_file; ++file)
        {
            add_legal_move(std::make_unique<En_Passant>(color_in, dir, file, en_passant_rank));
        }
